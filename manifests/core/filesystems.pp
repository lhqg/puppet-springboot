...
class springboot::core::filesystems (
  ...
  unless $log_vg =~ Undef {
    ...
    mount { '/var/log/springboot':
      device  => "/dev/mapper/${log_vg}-lv_log_springboot",
      require => [Filesystem["/dev/${log_vg}/lv_log_springboot"], Mountpoint['/var/log/springboot']],
      before  => File['/var/log/springboot'],
    }
  }
  ...
  file { '/var/tmp/springboot':
    seltype => 'springboot_tmp_t',
  }
  file { '/var/log/springboot':
